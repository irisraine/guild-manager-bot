...
class BandManager(commands.Cog):
    ...
    async def handle_role(self, interaction: nextcord.Interaction, member: nextcord.Member, action: str):
        ...
        band_leader_role_id = list(filter(lambda role_id: role_id in user_roles_ids, config.GROUP_LEADERS_ROLES))
        ...
        if action == "add":
            ...
            if band_role in member.roles:
                ...
                await interaction.response.send_message(embed=nextcord.Embed(
                    ...
                    description=f"Ковбой {member.mention} лишился членства в {band_role.mention}, и теперь он "
                                f"одинокий волк, которому никто не прикроет спину во время скитаний по прериям.\n\n "
